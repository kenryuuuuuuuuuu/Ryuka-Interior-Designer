...
const INTERIOR_DOORS = [
  ...
  { id:'door-029', type:'door-open', category:'door', operation:'open', label:'LDK⟷階段', orientation:'H', wallAt:1.82, center:14.106, width:0.91, height:2.4, floor:1, status:'estimated' }, // 新規(2026-08-15)：階段の実体表現にあたり新設。階段の直進部分（西側柱状部分）はLDKに向けて壁のない開口で開放されているリビング階段として配置。room-1f-10/room-1f-11の境界と一致させること。W07-H(2026-09-12)：汎用door-openの既定高さ2mで生成された垂れ壁が、正本13段を上る際の頭上に干渉することを実メッシュのレイ検査・UE歩行で確認。既存注記の「壁のない開口で開放されているリビング階段」に合わせ、室の既定天井高さ2.4mまで開放する仮設定を明示。実測・施工図に基づく確定寸法ではなくestimatedのまま施工確認待ち。幅・位置・段割りは変更しない。
...
const ROOMS_APPROX = {
  1: [
    ...
    { id:'room-1f-02', name:'玄関(民泊)', x0:0.91, x1:2.73, z0:0.91, z1:1.82, floorOffsetM:-0.1, conf:'低', note:'訂正(2026-08-13)：西端(x=0)と東端(x=7.280)を基準とした再キャリブレーションで、玄関の東壁はx=2.730が正しいと判明（前回のx=1.820は近接点同士のキャリブレーション誤差による誤り）。追記(2026-08-15)：施主指摘により南側1マス(2×1マス)を「ホール(民泊)」（room-1f-24）として分離' },
    ...
    { id:'room-1f-08', name:'自宅玄関', x0:9.1, x1:10.92, z0:0.455, z1:1.82, poly:[[9.1,0.455],[10.92,0.455],[10.92,1.82],[9.1,1.365]], floorOffsetM:-0.1, conf:'低', note:'施主指摘(2026-08-13)により赤枠の座標をピクセル解析。旧「自宅玄関・土間・ホール」から西半分を分離。2026-08-15：施主指摘により斜め框（目分量の角度）で「自宅玄関・ホール」を分割。角度は概算のため`estimated`。施主指摘により当初の(9.100,0.910)-(10.920,1.820)から(9.100,0.910)-(10.920,1.365)へ緩やかに変更後、同じ角度のまま0.455下（南）へ平行移動し(9.100,1.365)-(10.920,1.820)に' },
    ...
    { id:'room-1f-09', name:'土間', x0:10.92, x1:12.451, z0:0.455, z1:2.275, floorOffsetM:-0.1, conf:'低', note:'訂正(2026-08-13)：施主指摘により南端をz=2.730→2.275に縮小(0.5マス分をLD側へ移管)。2026-08-15：施主指摘により「土間・シューズクローク」を右の壁(x=13.651)から1200mmの位置(x=12.451)で分割し「土間」に改称。境界に壁はなく開口のみ（door-021）' },
    ...
    { id:'room-1f-18', name:'南土間', x0:16.38, x1:19.11, z0:6.37, z1:7.735, floorOffsetM:-0.1, conf:'高', note:'確定(2026-08-13)：施主指摘により3マス×1.5マス(2.730m×1.365m)で確定。z=6.370起点に修正（旧z0=5.460は誤り）' }
  ],
  2: [
