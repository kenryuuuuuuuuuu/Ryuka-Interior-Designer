...
2026-09-09更新。W06-v3（追加修正3ceda9bを含む）まで受入済みです（採光・照明は仮条件での機能確認）。**残りはW07〜W08の2段階**です。W07以降は事前計画（DRAFT）です。前段階の受入時に見直します。本書は古いVISUAL_TWIN_PLAN/STATUS内の進行順より優先します。

## 承認済みの実装順序

[詳細設計と見積](tasks/W07-staged-design.md)を作成しました。推奨は **ゲスト3単位→W08-Gで試用開始→自宅4単位→W08-Fで全館引渡し** です。大きな目的はW07/W08の2つですが、標準9件の実装・受入単位を見込みます。施主承認済みで、下記の当初一括計画よりこの順序を優先します。[W07-G1](tasks/W07-G1-multi-room-foundation.md)はREADYです。ゲスト試用版は標準4単位後です。
...
| 受入済み（仮条件） | [W05 採光](tasks/W05-daylight.md) | 日時・実敷地条件で日差しを比較する | 中 |
| 受入済み（仮仕様） | [W06 電気設備・照明](tasks/W06-electrical-lighting.md) | 器具配置を反映し、夜の点灯案を比較する | 完了 |
...
- 2026-09-09：W05-v2を仮条件での機能完成として受入。残りはW06〜W08。実敷地確認は別途未完了です。
