...
HUDは常時、操作キー一覧・現在の仕上げ名・太陽条件（日時由来か手動角度か、W05）・照明条件（昼間／夜間・仮仕様、W06）・直近操作の結果（保存/復帰の成否、復帰できない理由等）を日本語で表示します。追加のフォントアセットは使っていません（UE標準フォントで日本語グリフの描画を確認済み）。夜間の点灯・調光・色温度、日時ケース・敷地入力の操作は編集画面（Tools → 内装比較 → 照明／採光）から行い、内覧内には専用の操作UIを設けていません（W05/W06とも編集画面で条件を決めてから内覧で確認する運用）。
...
- 施主の内覧保存：`Saved/walkthrough-state.json`。既存の比較状態スキーマを共用します（現行2.0.0。W07-G1で複数室対応へ拡張し、部屋横断の項目（対象室・視点・太陽・露出・昼夜モード）と部屋ごとの`roomStates`（仕上げ・面別上書き・照明）を分離しました。旧1.0.0〜1.2.0は自動的に自室のみの`roomStates`を持つ2.0.0へ正規化され、古い保存の見た目を維持します）。歩行対象は今回もLDK単室のまま（`walkthrough.json`の`roomId`が指す部屋、W07-G2で室間移動に対応予定）ですが、保存・復元する状態は常に**スコープ全室分**の`roomStates`です。歩行対象室以外（例：洋室）のroomStateは、内覧の起動・F5保存・F9復帰を通じて一切変更されません（実機確認済み。詳細は[ARCHITECTURE.md「複数室対応」](ARCHITECTURE.md#複数室対応study-scopesjsonw07-g12026-09-09追加)）。
