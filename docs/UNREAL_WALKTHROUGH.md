...
ゲスト8室（玄関・ホール・LDK・洋室・トイレ・洗面脱衣・UB・収納）を実際に歩いて行き来し、7つの扉（引き戸2・開き戸3・両開き1・開放1）を開閉できることを実機で確認しました。`-RyukaSmoke`ネイティブ自己診断を**1本の連続スイープ**へ置き換え、玄関→ホール(door-025)→door-002開扉→LDK→ホール→door-002閉扉→door-005開扉→洋室→（洋室でFinish2→F5→ホールへ→door-005閉→F9で室・位置・扉・両室状態を復元）→洋室→ホール→door-001開扉→トイレ（水回り入口）到達、を確認。両開きdoor-024は両葉が洋室側へ開いて収納へ通行できることも確認。実Blenderビルド・実UEインポート（`unrealImportVerified: true`）・C++実コンパイル（`error C`/`error LNK` 0件）に加え、NullRHI（ロジックのみ）・DX12（実描画、`walkthrough-smoke.png`取得）の両方でPASS。UEエディタの`study_controls.py`検証（`verify_w07_g2.py`、29項目）で、`doorStates`が実際の扉パネルのtransformへ反映されること・エディタ視点変更で`walkthrough`帰属がその室へ再解決されること・仕上げA/B比較中に実葉が固定されること・旧案の通常読込で実葉が閉じること・引き戸の閉基準がエディタ保存＋レベル再読込を跨いでずれないことも確認しました。詳細な確認項目・コマンドは[W07-G2-report.md](tasks/W07-G2-report.md)を参照してください。

**レビュー対応（v1: R1〜R4、v2: R1〜R3）**：初回提出（`89d07af`）と v1対応版（`e7d6106`）がいずれも CHANGES_REQUESTED。同じG2で修正（BASE `60d8cd6` は不変）。v1: `doorStates` を実葉へ適用（Blender生成・UE初期インポート・`apply_state()`／案読込の全経路）、引き戸アウトセット化、エディタ視点変更時の `walkthrough` 処理、両開き符号、連続経路の実機確認。v2: **引き戸の閉基準を `door-bindings.json` の不変値 `closedLocationCm` にしてエディタ保存・再起動でずれないようにした**、**`select_room()`/`remember_view()` は `walkthrough` を「解除」でなくその室へ「再解決」して保存し、C++ `Restore()` は保存カメラの実XYが帰属先の室に入っているかも照合する（初回起動だけが玄関）**、**干渉判定に建物壁を戻し・開閉両方向を検査・開扉は衝突しない最大角へ制限（door-002 は75度で頭打ち）・閉扉は移動途中の施主/家具で拒否し、無効化して押し通す方式は撤去**、両開き符号の V壁向け修正で door-024 が洋室側へ開くようにした。詳細は[ARCHITECTURE.md「再レビューv2対応」](ARCHITECTURE.md#再レビューv2r1r3対応2026-09-10)。
