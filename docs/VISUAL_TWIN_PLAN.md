...
参考画像に基づく現在の実装順序と不足データは [ゲストLDK対応表](GUEST_LDK_REFERENCE_PLAN.md) を参照してください。テレビ本体と家具の設置高さ、ソファの詳細モデル対応は実装済みです。[詳細モデルの契約と次の作業](FURNITURE_ASSETS.md)に沿って主要家具を整えます。下記の全体構想と、既に実装済みの機能を区別して進めます。
