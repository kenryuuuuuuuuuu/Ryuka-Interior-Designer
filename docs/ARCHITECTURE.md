...
- **スポット照明の減衰・器具の自己遮蔽の修正（`unreal/import_study.py`、2026-09-16修正）**：施主報告（ダウンライトが暗く光源がそこから出ていないように見える／シーリングライトが自身の箱形状で光を遮っているように見える）を受けて2点修正した。(1) スポット光（ダウンライト）の`inner_cone_angle`と`outer_cone_angle`を同じ値にしていたため、フル輝度の範囲とゼロになる範囲の境界が完全に一致し、なだらかな減衰（ペナンブラ）が無い硬い光になっていた。`outer`は型の`spotAngleDeg`（全開き角）の半分のまま、`inner`をその6割に狭め、中心から縁にかけて自然に暗くなる帯を持たせた。(2) 照明の簡易プレースホルダー形状（`lighting.<id>.shade`/`.rod`、厚みのある不透明な箱）に影を落とさない設定（`set_cast_shadow(False)`、ガラス・階段当たり判定メッシュと同じ扱い）を追加した。全方位に光る無指向性光源（シーリングライト等）が箱の底面ぴったりに置かれているため、影を落としたままだと箱自身が光の大半を自己遮蔽していた。
  - **実機検証**：`build/lighting-fix-verify`で完全refreshし、`capture-unreal-study.py`で夜間モード・露出EV100=3.5の状態を2箇所レンダリングして目視確認した。①自宅LDK（room-1f-11、ダウンライト12灯点灯）：修正前に懸念された「暗い」状態は解消し、天井・壁面に自然なグラデーションの光だまりが出ている。②洋室（room-1f-05、シーリングライトelec-006単体点灯、天井を見上げる構図）：箱の周囲・下方とも遮られず大きく自然に光が回っており、自己遮蔽は解消された（至近距離から直視する構図のため中心付近はやや白飛びしているが、通常の視点では起きない想定）。画像は`build/lighting-fix-verify/ue/Saved/lighting-check-ldk.png`・`lighting-check-western-ceiling.png`（いずれもgit管理外のbuild/配下）。
  - **未対応のまま残した論点**：夜間モードが昼間用の露出EV100をそのまま引き継ぐ既知の問題（[BACKLOG.md](BACKLOG.md)既載、目安値EV3.5は今回の検証キャプチャでも踏襲）は、今回のフレーミング固有の値で検証しただけで、状態遷移（昼→夜切替時に自動でEVを下げる等）そのものは変更していない。実際の内覧（歩行しながらの見え方、L字キーでの点灯操作、他の光源との組み合わせ）は静止画キャプチャの範囲外のため未確認。
- **夜間専用露出・ダウンライト100形化（`data/visual/lighting-settings.json`・`unreal/lighting.py`・`unreal/study_controls.py`・`Walkthrough.cpp`、2026-09-16〜17）**：上記の実機検証を施主が実際の内覧（脱衣室、夜間モード）で確認したところ、「ダウンライト周辺だけ暗い」という別の見え方が再現し、原因調査の結果、**器具の光束自体は現実的な下限（60形650lm）だったが、UEの固定露出が常に昼間の値（既定EV100=7.5、約200〜250lxを中間グレーとする設定）を夜間もそのまま使っており、100〜150lx程度の室内照明では実際より暗く写っていた**ことが判明した（施主指摘：「不気味なほど暗い」「100形にする」）。3点対応した。
  - **夜間専用のEV100**：`lighting-settings.json`に`nightExposureEV100`（既定5.5、約60lxを中間グレーとする設定、人の目が室内に順応した見え方に近い近似）を新設。`unreal/lighting.py`の`night_exposure_ev100(settings, day_ev100)`が「設定にあれば夜間専用値、無ければ昼間の値をそのまま」を返す（値が無い/不正な場合は既存動作のまま、後方互換）。`study_controls.apply_state()`（エディタ）と`Walkthrough.cpp`の`ApplyConditions()`（内覧、`LightingSettings`を新規ロード）の両方が、`lighting.mode==night`のときだけこの値でポストプロセスの固定露出を上書きする。**保存状態(`study-state.json`)自体の`exposureEV100`は昼間の値のまま変更しない**（`scene_state()`は夜間中もEVを読み戻さず`base['exposureEV100']`を素通しする、既存のsunLux/器具状態と同じ「Undo等の不整合検知」の考え方は踏襲）。昼へ戻せば自動的に昼間の値へ戻る。
  - **ダウンライトを100形へ**：`light-downlight`/`light-downlight-sensor`を650lm→1,500lm（100W相当）、`spotAngleDeg`を90°→120°（拡散タイプの公称配光角）に変更。`import_study.py`側もinner_cone_angleを`outer*0.6`→`outer*0.5`に広げ、公称角の外へ緩やかに漏れる実物寄りの配光にした（総光量はUEのlumens指定により円錐の広さに関わらず一定）。
  - **実機検証**：`build/lighting-fix-verify-v2`で完全refresh（`--previous build/lighting-fix-verify/ue`）し、脱衣室（room-1f-16、施主が実際に暗いと報告した部屋・構図）を`capture-unreal-study.py`で再レンダリングして目視確認した。夜間モードで`exposureEV100`は状態の昼間値のまま渡し、`nightExposureEV100`が自動適用されることを確認（内覧・エディタと同じ経路）。施主が同じ構図を内覧で確認し「いい感じ」と承認済み。画像は`build/lighting-fix-verify-v2/ue/Saved/lighting-check-datsuisitsu-v2.png`（build/配下、git管理外）。
  - **検証テスト**：`tests/test_lighting.py`に`night_exposure_ev100()`の単体テスト（設定あり/なし/不正値、リポジトリの実ファイルが有効な値を持つこと）を追加。
  - **未対応のまま残した論点**：EV5.5・ダウンライト100形・配光120°は一般的な目安値であり、実採用品番・実測照度への校正はしていない（既存の「電気設備」節の限界と同じ扱い）。子供部屋・寝室等のシーリングライト（3800lm、変更なし）や他の器具クラスとの相対バランスは未検証。ダウンライトを60形で使う部屋が今後出てきた場合、現状は全ダウンライトが一律100形になる（型を分けたい場合は別途相談）。
- **人感ダウンライト（`light-downlight-sensor`、2026-09-14追加）**：施主指示により、`data/electrical-catalog.json`へ新設した型（mount:ceiling、光学プロファイルは`light-downlight`と同一）。玄関・土間系4器具（elec-002 玄関(民泊)／elec-011 自宅玄関／elec-013 土間／elec-029 南土間）を`light-downlight`から変更した。既存の壁スイッチ（土間の`switch-sensor`含む）はそのまま残置しており、このダウンライト専用の操作対象ではなくなっている（施主要望の範囲外のため削除していない。要不要は施主判断）。データ・Web編集側は既存の型カタログ機構にそのまま乗るため追加実装は無い（`ELECTRICAL_CATALOG`はJSONから動的生成、`shape`も既存の`downlight`関数を流用）。
  - **UE内覧での自動点灯（`Walkthrough.cpp`の`ApplySensorLights()`）**：`lighting-bindings.json`の各fixtureが常に持つ`type`フィールド（`build_lighting_bindings()`が元から書き出している）で`light-downlight-sensor`を判別し、対象室に入った時点でその部屋のセンサー器具のうちoffのものだけをon にする。実装は`ToggleRoomLights()`（Lキー手動トグル）と同じ「roomStates[room].fixtures[id].onを書き換えてApplyConditions()を通す」経路の流用で、失敗時は元のroomStateへ静かに戻す（ApplyConditions()は成功するまでシーンを変更しないため、直前の適用済み状態のまま矛盾は残らない）。呼び出し箇所は`UpdateCurrentRoom()`の室遷移2か所（階段室・隣接室への移動）と`Restore()`の成功末尾（初回スポーン・F9復元・案読込を「入室」として扱う）。`IsCurrentRoomEditable()`（仕上げ編集scope＝LDK・洋室のみ）とは無関係に動く。**自動消灯は未実装**（施主指示は「入ったら点く」のみのため、範囲外として意図的に見送った。退室時に消したい場合は追加相談が必要）。
  - **検証状況（2026-09-16更新）**：このマシンにBlender・Unreal Engineが実際にインストール済みであることを確認し（当初「この環境には無い」としていたのは未確認による誤り）、`scripts/refresh-visual-study.py`（`build/lighting-fix-verify`）で完全refreshを実行、`ApplySensorLights()`を含む`Walkthrough.cpp`一式のC++コンパイルが成功することを確認した。ただし「玄関・土間系4室への入室で実際に点灯するか」という歩行時の動作そのものは、静止画キャプチャ（`capture-unreal-study.py`）では検証できないため未確認のまま。次に実際に内覧を歩いて確認する必要がある。
...
**2026-09-14の改訂：** 上記の「室の奥行で開く側を決める」は現在の仕様ではありません。Three.jsは室内ドアの`outSign=+1`で`swingDir`を描画しており、UEも`data/interior-doors.json`の`swingDir`（`out`は壁法線の正方向、`in`は負方向）を正本として使います。未指定の旧データだけ室の奥行へフォールバックします。固定壁との安全角は選ばれた側の室境界で再計算します。変更前に相違していた開き戸はdoor-007/008/012/014/015/016/018です。
...
- **文書化した固定幾何の限界（v1時点の記述。v2対応で更新→次項）**：door-024・door-002の開き角制限。

### 再レビューv2（R1〜R3）対応（2026-09-10）

v1対応版（`e7d6106`）も再レビューでCHANGES_REQUESTED。同じG2で以下を修正した（BASE `60d8cd6` は不変）。

- **R1：引き戸の閉基準を「不変の記録値」にした**。v1は「いま見えている葉の位置 − `bakedOpen`分」を閉基準にしていたため、閉生成した引き戸を開いて**エディタ保存（`save_current_level()` が `.umap` へ葉位置も保存）→再起動**すると、開位置を閉基準と誤認し、以後の開閉で1枚分ずつずれた。修正：`import_study.py` が生成直後の（確実に生成ポーズの）シーンで各引き戸葉の**閉ワールド座標を一度だけ捕捉**し、プロジェクトの `door-bindings.json` に `closedLocationCm` として書き込む。`study_controls.apply_state()` と C++ `ApplyConditions()` はこの固定値を参照して絶対位置をセットするだけになり、ライブポーズからの毎セッション推定（`_door_leaf_closed_location`／`DoorLeafSpawnLocation` のキャッシュ）は撤去した。
