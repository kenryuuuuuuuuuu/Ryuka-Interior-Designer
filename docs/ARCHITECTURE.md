...
- **検証**：実Blenderビルド・実UEインポート（`unrealImportVerified: true`）・C++ Walkthroughモジュールの実コンパイルに加え、`-RyukaSmoke`ネイティブ自己診断を拡張し、玄関(非編集室)から始めた場合の仕上げキー省略に加え、**引き戸（door-001）・開き戸（door-002）それぞれについて、閉状態での通行ブロック・状態トグル・開状態での通行成功を実際のCharacterMovementスイープで確認**（AC2/AC3）、**洋室相当の編集可能室で扉を開けてF5→別室へ移動して扉を閉じ直す→F9で室・位置・扉状態（開）を復元**（AC4）を確認した。NullRHI（ロジックのみ）・DX12（実描画、スクリーンショット取得）の両方でPASSを確認。UEエディタでの`study_controls.py`検証（15項目）：`doorStates`の初期値・モデル存在チェックによる拒否・仕上げA/B比較中の`doorStates`固定・名前付き案読込による`doorStates`の家全体丸ごと採用、すべて確認。詳細・実行コマンドは[W07-G2-report.md](tasks/W07-G2-report.md)を参照。
