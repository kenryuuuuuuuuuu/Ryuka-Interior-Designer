...
### キッチン・冷蔵庫の形状（2026-09-06）

Blenderの家具生成に、シンクの底・側面、シンクとコンロを避けた天板、収納扉・手掛け、冷蔵庫の上下扉を追加しました。品番未確定の仮形状です。シンク深さは最大160mm（低い家具では高さの20%）、天板厚は最大35mmとし、配置と外寸は家具正本を使います。水栓・配管・排水口・実製品の扉機構は未実装です。

部材には元の家具JSONをそのまま残し、細部を`estimated`と記録します。家具を回転・移動した際は全パーツが同じ変換に従います。`validate_interior_scene.py`でシンク底へ光線が届くこと、コンロが天板で覆われないこと、各パーツが元の外寸内であることを確認し、GLB往復とUE取り込みで外形を照合します。

**向きの確認待ち**：`fur-006`（ゲストLDKキッチン）と`fur-007`（冷蔵庫）は現在`rotation:90`です。生成形状の正面（ローカル+z）が図面東側の壁を向くため、室内からは主に背面が見えます。Three.jsのキッチンもローカル-zを背面とする形状です。室内側を正面とするか施主に確認中で、正本の回転値は変更していません。採用品番と正面方向の確定後に整えます。
