...
- **HEAD**：`ab1d4a8`（`W07-G3: guest 8-room interiors`〔63c8afd〕→ `W07-G3: add scopeIds circulation test`〔31c78d0〕→ `docs: record W07-G3 HEAD SHA`〔e28dbc7〕→ 本コミット〔昼夜代表画像の証跡記載〕。本行SHAは直後のdocsコミットで確定）
...
- **代表プロジェクト**：`build/W07-G3-blender-v1` → `build/W07-G3-ue-v2`（`unrealImportVerified: true`）、完全refresh `build/W07-G3-refresh-v1`。
...
- 面：**合計 52 面すべて解決・bind**（`surface-registry.py` の resolve、`build/W07-G3-ue-v2/surface-bindings.json` の全 status=bound）。6室はいずれも house.json の 4 頂点矩形・フラット天井。
...
- **空室の対応（W07-G3で修正した1件）**：玄関/ホール/収納（および家具1点のトイレ/UB）は whole-scene の役割スロット幾何を持たないため、`study_controls.scene_state()` が室の variant を導出できず基準variantへ固定していました。登録済み壁/床/天井の**サーフェスマーカー（`M_Surf_<id>_<variant>`）から実 variant を読み戻す**よう修正（`unreal/study_controls.py`）。エディタ保存・再読込後も空室の仕上げが正しく往復します。
...
- `build-visual-twin.py --interior --scope guest --output build/W07-G3-blender-v1`：GLB 往復成功。`study.json` の scopeId=guest・8室、`surface-bindings.json` 全 52 面 bound、`lighting-bindings.json` 10 灯。
- `build-unreal-study.py ... --output build/W07-G3-ue-v2`：`unrealImportVerified: true`（709 メッシュ、maxBoundsErrorCm < 1e-3）。
- `enable-unreal-walkthrough.py`：`walkthrough.json` は `guest-circulation`（8室・7接続・開始室 玄関）を解決。C++ ソース変更なし（`error C`/`error LNK` 0）。
- `-RyukaSmoke`（NullRHI・DX12）：G2 の連続経路（玄関→ホール→door-002→LDK→…→トイレ）＋AC4＋R2＋R3が **PASS**（歩行は scope 非依存、`walkthrough-smoke.png` 取得）。
- `verify_w07_g3.py`（`-run=pythonscript`）：全項目成功（上記 AC 根拠）。
- 完全refresh 1 回：`status: complete`、終了コード0。
...
python scripts/build-visual-twin.py --blender "C:/Program Files/Blender Foundation/Blender 5.2/blender.exe" --interior --scope guest --output build/W07-G3-blender-v1
python scripts/build-unreal-study.py --engine "C:/Program Files/Epic Games/UE_5.8" --package build/W07-G3-blender-v1 --output build/W07-G3-ue-v2 --cache C:/UE_DDC/w07g3b
python scripts/enable-unreal-walkthrough.py --project build/W07-G3-ue-v2 --engine "C:/Program Files/Epic Games/UE_5.8" --cache C:/UE_DDC/w07g3b
python scripts/launch-unreal-walkthrough.py --project build/W07-G3-ue-v2 --engine "C:/Program Files/Epic Games/UE_5.8" --cache C:/UE_DDC/w07g3b --smoke
# UEエディタ検証：UnrealEditor-Cmd.exe <v2>/RyukaInterior.uproject -run=pythonscript -script=<v2>/verify_w07_g3.py -unattended -NullRHI ...
python scripts/refresh-visual-study.py --previous build/W07-G3-ue-v2 --output build/W07-G3-refresh-v1 --blender "C:/Program Files/Blender Foundation/Blender 5.2/blender.exe" --engine "C:/Program Files/Epic Games/UE_5.8" --cache C:/UE_DDC/w07g3refresh --scope guest
```

### 証跡ファイル

- `build/W07-G3-ue-v2/verify_w07_g3.py`・`verify_w07_g3_result.json`（`_all_passed: true`）
...
- `blender/furniture_assets.py`：`toilet_parts`/`vanity_parts`/`washer_parts`/`bathtub_parts` を新設・登録
- `unreal/circulation.py`：`validate_profiles`/`resolve_profile_for_scope` が `scopeIds`（配列）を受け付け、scope重複主張を拒否
- `unreal/study_controls.py`：`scene_state()` が空室（役割スロット無し）の variant をサーフェスマーカーから読み戻す
- `tests/test_circulation.py`：`scopeIds` の解決テスト、`tests/test_surface_registry.py`：52面 resolve に更新
- `docs/ARCHITECTURE.md`・`docs/STATUS.md`・`docs/tasks/W07-staged-design.md`：G3の内容を反映

## 不足・未決定一覧（仮仕様・観測値を推測で上書きしていない箇所）

1. **残り6室の初期視点**（`room-render-settings.json`）は仮設定。実運用時の見え方は未確認。室ごとに調整可能。
