...
| ローカル敷地情報（`site.local.json`、緯度経度・真北・確度） | あれば保持します。無い旧案でも太陽条件・日時ケースが有効なら引き続き使えます（W05、2026-09-09追加） |
...
## ローカル敷地情報・日時比較（W05、2026-09-09追加）

UEエディタの「内装比較 → 採光」メニューで、緯度経度・真北・確度（`estimated`/`confirmed`）を持つローカルの敷地情報（`site.local.json`）を読み込み・新規作成できます。実敷地の測量・仕様が未確定な間は`estimated`のまま仮条件として使い、確定後に確度を上げて再保存します。この情報は常にローカル専用（`.gitignore`対象）で、正本・リポジトリには含めません。

同メニューから、季節代表日時（3/21・6/21・9/21・12/21の各9/12/15時、暦上の代表日であって天文学上の正確な二至二分ではありません）や任意の日時を`sun-cases.json`へ追加できます。敷地情報を変更しても既存の日時ケースは自動再計算しません（「現在の敷地と不一致・再計算が必要」と表示されるだけです）。意図せず古い敷地の値のまま比較しないよう、明示的な再計算操作が必要です。

「内装比較 → 日時比較」メニューでは、仕上げ・視点・露出・光源強度・周辺条件を固定したまま、太陽角度・日時だけを切り替えるA/B比較ができます（W04の仕上げA/Bとは別の比較です）。比較対象の両案は、切り替える前に必ず面参照・周辺条件の適合を確認し（`_context_compatible`と面上書きの参照確認を合わせた事前チェック）、片方でも不適合ならどちらの案も適用せず中止します。日没・日の出前後で使用不可（`usable: false`）と判定されたケースは一覧には残ります（理由付き）が、適用・比較開始のどちらも拒否されます。

`site.local.json`は名前付き案（`build/scenarios/...`）にも他の入力（状態・日時ケース・周辺条件）と同様に同梱されます。旧来の敷地情報を持たない案は今も有効です（`sun-cases.json`と`site.local.json`をあわせて同梱した場合のみ、両者の対応が厳密に照合されます）。`build-unreal-study.py`／`refresh-visual-study.py`はいずれも`--site <path>`を受け付け、`--sun-cases`と組で渡す場合は両者の対応を確認してから取り込みます。

固定した仕上げ・視点・露出で日時だけを変えた撮影比較には、`compare-unreal-studies.py`（仕上げ×日時の一括比較、`--variant`必須）ではなく`compare-unreal-daylight.py`（`--sun-case`のみを渡し、現在の仕上げ・視点・露出を変えずに複数の日時を撮影）を使います。
