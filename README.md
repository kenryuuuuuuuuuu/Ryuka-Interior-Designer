...
- `data/electrical.json` — 電気設備の配置（どこに何を置くか）。正本。第1段階（データ構造・表示）のみ完了、Web UI編集は未実装
...
- 平面図モードで右上「✎ 電気設備編集」：電気設備をクリック/タップで選択し、壁付け設備（コンセント・スイッチ等）は壁に沿ってドラッグでスライド、天井/床付け設備（照明等）は家具と同じ自由なドラッグで移動。選択中に表示されるパネルから種類切替・幅/奥行/高さ/取付け高さの変更・向きの反転ができる。「electrical.jsonを書き出す」でダウンロードしたJSONを`data/electrical.json`に上書きしてコミットするまでは正本に反映されない。家具編集・窓ドア編集とは同時にONにできない
