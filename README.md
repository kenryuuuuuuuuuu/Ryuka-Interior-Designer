...
家具編集はこのフォルダの **interior-white-model.html**、UE内覧・更新は **interior-launcher.cmd** を開きます。旧build/worktrees配下は使いません。[フォルダと手順](docs/DAILY_USE.md)／[残課題・確認待ち事項](docs/BACKLOG.md)。
...
保存した内装検討をまとめて更新する場合は[一括更新の手順](docs/REFRESH_WORKFLOW.md)を参照してください。

Blender＋Unrealで進める内装・採光検討の方針と再生成手順は [docs/VISUAL_TWIN_PLAN.md](docs/VISUAL_TWIN_PLAN.md) を参照してください。ゲストLDKのBlender比較画像、UE 5.8.2への検証付き取り込み、エディタの「ツール → 内装比較」から仕上げ・日時による太陽位置・視点を切り替え、条件を次回生成へ引き継ぐ機能を実装しています。所在地と真北はローカル設定で管理します。全館の歩行UIはW07/W08-Fで実装済みです。実敷地の条件確定と照度の校正は確認待ちです。
...
- 俯瞰モードで右上「✎ 電気設備編集」：ONにすると画面右に編集専用ドックが開く。ドック上段の部屋選択セレクトで部屋フォーカスすると、その部屋だけを表示し他室の壁・家具・電気設備・ドア窓・防音壁・腰壁を隠してカメラを寄せる（電気設備のラベルは表示チェックボックスの状態に関わらずこの部屋だけ常に見える）。電気設備は狙った器具にマウスオーバーしたときだけカーソルが変わり、クリックで選択できる（近くの器具を誤って選んでしまうことはない）。選択中はドック中段の編集パネルから種類切替・幅/奥行/高さの変更・向きの反転・X/Y/Zの数値入力による微調整（壁付け設備は壁に沿う側の座標だけ編集でき、壁面側は固定・編集不可）ができるほか、「複製」で同じ壁/部屋にもう1つ増やす、「高さ(Y)を標準に戻す」でY座標だけ型の標準値に戻す、「🗑 この設備を削除」で削除できる（削除は取り消せる下書き）。壁付け設備は「設置している壁」セレクトから別の壁面へ丸ごと移動でき、選択中の壁は3D上で黄色くハイライトされる。Delete/Backspaceキーで削除、矢印キーで微調整、Escapeで選択解除、Ctrl+Z/Ctrl+Shift+Zで元に戻す/やり直すこともできる。このセッションで変更・追加した設備には常時うっすらとした色付きリング（変更＝水色、追加＝緑）が付き、どこを触ったか一目で分かる。新規追加はドック上段の「＋ この部屋に設備を追加」から行い、壁付け設備はその部屋に接する壁面を選んでから追加する（追加後にどの座標がロックされているか分かるように、先に壁面を決める設計）。ドック上段の「一覧を表示」からは、見積書の明細ごとの現在数/見積数（差があれば強調表示）と1F/2F全部屋（設置なしの部屋も含む）ごとの内訳を一覧表示でき、行をクリックするとその設備の部屋へフォーカスして選択、削除した設備は末尾の「削除した設備」欄からいつでも個別に復元できる。ドック下段の「元に戻す/やり直す」ボタンと「electrical.jsonを書き出す」ボタン（ダウンロードしたJSONを`data/electrical.json`に上書きしてコミットするまでは正本に反映されない）もここにある。平面図では電気設備が小さく視認・選択しづらいため俯瞰モード専用にしている。家具編集・窓ドア編集とは同時にONにできない
