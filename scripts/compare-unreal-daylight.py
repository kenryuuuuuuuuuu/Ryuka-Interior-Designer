"""Capture a fixed-finish, fixed-camera datetime comparison and a local review gallery.

Unlike compare-unreal-studies.py (which captures a solar-case x variant
MATRIX, forcing a specific --variant on every shot), this fixes the CURRENT
comparison state's finish -- including any W04 per-surface overrides -- and
only switches the sun-case between shots, matching the W05 "日時比較"
feature: same view/exposure/light intensity/finish, only the sun position
differs. See docs/tasks/W05-daylight.md section 3.
"""
import argparse
import hashlib
import html
...
def check_capture(report, case, baseline):
    ...
    if not report.get('levelFileUnchanged') or not matches(case,state):
...
def gallery(document):
    ...
    for entry in document['captures']:
        ...
        cards.append(f'<figure><a href="{esc(entry["image"])}"><img src="{esc(entry["image"])}" alt="{esc(solar["localTimestamp"])}"></a>'
            f'<figcaption><strong>{esc(solar["localTimestamp"])}</strong>'
    ...
    return ('<!doctype html><html lang="ja"><meta charset="utf-8"><meta name="viewport" content="width=device-width">'
        '<title>ゲストLDK 日時比較</title><style>body{font-family:system-ui,sans-serif;background:#f3f1ed;color:#292722;margin:0;padding:24px}'
        ...
        '<main><h1>ゲストLDK 日時比較</h1><p><strong>'+esc(document.get('note','実敷地との対応は未確認です。'))+'</strong></p>'
        '<p>同じ視点・光源強度・露出・仕上げ（面別上書きを含む）で、日時（太陽位置）だけを比較しています。</p>'
        '<p>天候・ガラス透過率・室内照度は未校正です。所在地に関わる条件を含むため、このフォルダはローカルで保管してください。</p>'
...
def main():
    ...
    parser.add_argument('--sun-cases',type=int,nargs='+',required=True,help='At least 2 unique, usable zero-based case indices')
    ...
    indices=args.sun_cases
    if len(indices)<2 or len(set(indices))!=len(indices) or any(not 0<=i<len(cases) or not cases[i]['usable'] for i in indices):
        parser.error('Choose at least 2 unique, usable solar case indices.')
    if len(indices)>12: parser.error('Choose at most 12 datetimes per batch.')
    ...
    if not (project/'study-state.json').exists(): parser.error('Rebuild this project with the comparison controls first.')
    before=fingerprint(project)
    prefix='daylight-'+uuid.uuid4().hex[:12]
    ...
    for i in indices:
        document['captures'].append(dict(caseIndex=i,name=f'{prefix}-{i}',status='pending'))
    output.mkdir(parents=True); write_manifest(output,document)
    baseline=None
    try:
        for count,entry in enumerate(document['captures'],1):
            print(f'[{count}/{len(document["captures"])}] case {entry["caseIndex"]}',flush=True)
            if fingerprint(project)!=before: raise RuntimeError('Project changed during comparison')
            # No --variant: this is what keeps the current finish (including
            # any surfaceOverrides) fixed instead of forcing a named variant.
            subprocess.run([sys.executable,str(ROOT/'scripts/capture-unreal-study.py'),'--engine',str(args.engine),
                '--project',str(project),'--cache',str(args.cache),'--name',entry['name'],
